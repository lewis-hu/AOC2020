...
for (int i = 0; i < arr.Length; i++)
{
	...
	for (int j = 0; j < arr.Length; j++)
	{
		...
		for (int k = 0; k < arr.Length; k++)
		{
			...
			{
				numberFound = true;
				firstNumber.Dump(nameof(firstNumber));
				secondNumber.Dump(nameof(secondNumber));
				thirdNumber.Dump(nameof(thirdNumber));
				(firstNumber * secondNumber * thirdNumber).Dump("multiplied");
				break;
			}
		}
