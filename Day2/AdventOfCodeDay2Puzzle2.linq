...
static async Task Main()
{
	...
	foreach (var input in inputList)
	{
		...
		//range.Dump("Range");
		var firstChar = int.Parse(range[0]) - 1;
		var secondChar = int.Parse(range[1]) - 1;
		var charToFind = entry[letter][0];
		//charToFind.Dump("character to find");
		//entry[password].Dump("Password");
		var firstMatch = entry[password][firstChar];
		var secondMatch = entry[password][secondChar];
		//matchCount.Dump("Match Count");
		if ((firstMatch == charToFind || secondMatch == charToFind)
			&& !(firstMatch == charToFind && secondMatch == charToFind))
		{
			validCount++;
			"valid".Dump();
			continue;
